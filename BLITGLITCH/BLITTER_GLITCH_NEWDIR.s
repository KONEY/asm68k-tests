...
__BLIT_GLITCH_DATA:
	...
	MOVE.W	#bpl-wblit,BLTAMOD		; BLTAMOD =0 perche` il rettangolo
	MOVE.W	#bpl-wblit,BLTDMOD		; BLTDMOD 40-4=36 il rettangolo

	MOVE.L	A3,BLTAPTH		; BLTAPT  (fisso alla figura sorgente)
	MOVE.L	A4,BLTDPTH
	MOVE.W	#32*64+(w_real/2)/16,BLTSIZE	; BLTSIZE (via al blitter !)
	; ## MAIN BLIT ####
	ADD.L	#8,GLITCHDATA
	RTS

__SCROLL_BG_X:
	MOVE.B	TOT_SHIFT_FACTOR,D1
	ADD.B	#1,D1
	AND.B	#127,D1
	MOVE.B	D1,TOT_SHIFT_FACTOR
	TST.B	D1
	BNE.B	.dontDrop
...
__SCROLL_BG_Y:
	...
	MOVE.W	#(h-3)*64+(w_real/2)/16,BLTSIZE	; BLTSIZE
	; ## MAIN BLIT ####
...
__SCROLL2_BG_X:
	...
	ADD.L	#(w/2/16*2),SCROLL_PLANE
	ADD.L	#(w/2/16*2),SCROLL_SRC

	CMP.B	#1,D5
	BEQ.B	.mainBlit
	; ## FOR LEFT ####
	SUB.L	#(w/2/16*2),SCROLL_PLANE		; THIS IS...
...
__BLOCK_5:
	; 5: Robot + Ritmo + KassaIPERImb
	MOVE.W	#2,COPPER\.BPL1MOD		; FIX GLITCH
	MOVE.W	#2,COPPER\.BPL2MOD		; RESET
...
__BLOCK_6:
	...
	MOVE.W	AUDIOCHLEVEL0,COPPER\.BPL1MOD	; GLITCH BY MESSING WITH BPLxMOD
	ADDQ.W	#2,COPPER\.BPL1MOD		; TO END IN INITIAL VALUE

	MOVE.W	#2,COPPER\.BPL2MOD
	MOVE.W	P61_1F,D2			; 1Fx
	CMPI.W	#2,D2			; 1F2
	BNE.S	.dontGlitch
...
__BLOCK_7:
	...
	MOVE.W	BPLxMOD_INDEX,D7
	LEA	BPLxMOD_TABLE,A3
	MOVE.W	(A3,D7.W),COPPER\.BPL2MOD	; GLITCH BY MESSING WITH BPLxMOD
...
__BLOCK_VOX:
	...
	MOVE.W	P61_rowpos,COPPER\.BPL1MOD	; GLITCH BY MESSING WITH BPLxMOD
	ADDQ.W	#2,COPPER\.BPL1MOD	; TO END IN INITIAL VALUE
	...
	MOVE.W	P61_visuctr0,COPPER\.BPL2MOD	; OK I'M CRAZY :D
...
__BLOCK_9:
	...
	MOVE.B	SCROLL_DIR_1,SCROLL_DIR_X
	MOVE.L	BGPLANE1,SCROLL_PLANE	; INVERT
	MOVE.B	AUDIOCHLEVEL1,SCROLL_SHIFT
	BSR.W	__SCROLL_BG_X		; SHIFT !!

	MOVE.W	AUDIOCHLEVEL0,COPPER\.BPL2MOD	; GLITCH BY MESSING WITH BPLxMOD
	ADDQ.W	#2,COPPER\.BPL2MOD	; TO END IN INITIAL VALUE

	MOVE.W	#2,COPPER\.BPL1MOD
	MOVE.W	P61_1F,D2			; 1Fx
	CMPI.W	#2,D2			; 1F2
	BNE.S	.dontGlitch
	...
	MOVE.W	BPLxMOD_INDEX,D7
	LEA	BPLxMOD_TABLE,A3
	MOVE.W	(A3,D7.W),COPPER\.BPL1MOD	; GLITCH BY MESSING WITH BPLxMOD
...
__BLOCK_C:
	...
	MOVE.W	AUDIOCHLEVEL1,COPPER\.BPL1MOD	; GLITCH BY MESSING WITH BPLxMOD
					; SOME EXTRA MESS
	MOVE.W	D2,COPPER\.BPL2MOD	; GLITCH BY MESSING WITH BPLxMOD
...
__BLOCK_D:
	...
	ADDQ.W	#2,D3			; TO END IN INITIAL VALUE
	MOVE.W	D3,COPPER\.BPL1MOD	; GLITCH BY MESSING WITH BPLxMOD
...
BG1_DATA:		DS.B h*bwid	; INCBIN "BG_KONEY_DEMO_AMIGA_3.raw"
...
COPPER:
	.Palette:
	DC.W $0180,$0000,$0182,$0444,$0184,$0232,$0186,$0333
